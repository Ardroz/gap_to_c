  ...
  switch ( parameters.block.BlockType ) {
    ...
    case "I_TO_AN":
      ...
      while(cont != 0){
        ...
        code = code.concat( stringOutputs ).concat (' = (double)').concat( stringInputs ).concat( ';\n')
        cont--;
      ...
      parameters.fields.forEach( function ( element, index, array ) {
        if ( element.IOType === 'Input' || element.IOType === 'Tune' ) {
          if ( element.FieldName.substring(0,7) === 'SF_SEL_' ) {
            ...
            tcond.push( element.Value.replace(/\56/g,"_") );
          }
          
        } else if ( element.IOType === 'Output' ) {
          outputValues.push( parameters.block.Category + '_' + parameters.block.Name + '_' + element.FieldName);
          outputinputs.push( parameters.block.Category + '_' + parameters.block.Name + '_' + element.FieldName);

        }
      });
      tcond = tcond.slice(2,tcond.length);

      outputinputs = outputinputs.splice(2,outputValues.length);
      stringInputs = inputValues.join( ' || ' );
      code = 'if('  + stringInputs + ') { \n\t';
      tcond.forEach( function ( element, index, array ) {
        code += 'if( ' + element + ') {\n';
        code += '\t\t' + outputinputs[index] + ' = ' + element + ';\n';
        code += '\t} else ';
      });
